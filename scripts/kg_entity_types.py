...
{{
  ...
  "anchors": [
    {{
      "type": "key_judgment|key_data|comparison|causality|process_step|metaphor",
      "content": "锚点描述（15-40字）",
      "relevance": 0.9
    }}
  ]
...
除了实体和关系，还要提取笔记中的「认知锚点」——让一篇笔记值得读的核心节点。

### 6种锚点类型

| 类型 | 含义 | 示例 |
|:-----|:-----|:-----|
| key_judgment | 核心判断 | "资金因子在成长市场IC=0.63" |
| key_data | 关键数据 | "IC衰减率月均-3.2%" |
| comparison | 对比关系 | "北向资金vs主力资金流向背离" |
| causality | 因果链 | "信息不对称→聪明钱→价格发现" |
| process_step | 流程步骤 | "S01采集→S02因子→S03信号" |
| metaphor | 隐喻/类比 | "因子像筛子，不同市况筛不同股票" |

### 锚点提取原则

- 每篇笔记提取 1-4 个锚点，只选最重要的
- 锚点必须能从原文中找到依据，不编造
- content 控制在 15-40 字，一句话说清
- relevance 0-1 表示重要度，最强锚点给 1.0
- 不要和实体/关系重复——锚点是「为什么这段重要」而非「有什么概念」
